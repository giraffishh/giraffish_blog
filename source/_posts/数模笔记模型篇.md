...
假设标准化矩阵 $Z = [z_{ij}]$，每列是归一化的值，每列的权重为 $w_j$，则加权后的矩阵 $V = [v_{ij}]$ 中的每一个元素为：
...
正理想解 Z+: [0.669, 0.919, 0.976, 0.894]
负理想解 Z-: [0.446, 0.000, 0.000, 0.000]
...
def topsis(decision_matrix, weights, criteria_specs, alternatives_labels):
    ...
    """
    print("--- Intermediate Step 1: Forwardized Matrix (Positive Matrix) ---")
    ...
    print("--- Intermediate Step 2: Normalized Matrix ---")
    # Step 2: Normalize the forwardized decision matrix
    norm_matrix = positive_matrix / np.sqrt(np.sum(positive_matrix**2, axis=0))
    print(norm_matrix)
    print("\n" + "="*50 + "\n")


    print("--- Intermediate Step 3: Weighted Normalized Matrix ---")
    ...
    print("--- Intermediate Step 4: Ideal and Negative-Ideal Solutions ---")
