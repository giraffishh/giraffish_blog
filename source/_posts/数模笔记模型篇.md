...
<iframe width="1120" height="630" src="https://www.youtube.com/embed/o_cAOa5fMhE?si=22TFQYqdtT-ZSKHF" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
> 为什么样本方差/协方差的分母是 m-1？
>
> 在统计学和数据分析中，当我们计算一个样本的方差或协方差矩阵时，一个常见的问题是：为什么公式的分母是样本数量 `m` 减 1 (`m-1`)，而不是 `m`？
>
> 简而言之，答案是：**使用 `m-1` 是为了得到对总体方差的“无偏估计”。**
>
...
> 2. 偏差的来源：为何除以 `m` 会出错？
>
> 如果我们拥有整个总体的数据，并且知道总体的真实均值 $\mu$，那么计算总体方差的公式确实是除以 `m`：
>
> $$
...
>
> $$\sum_{i=1}^{m}(x_i - \bar{x})^2 \le \sum_{i=1}^{m}(x_i - \mu)^2$$
>
> 这是因为样本均值 $\bar{x}$ 本身就是为了让这个平方和最小而计算出来的“中心点”。因此，如果我们用 $\bar{x}$ 来计算方差，并且仍然除以 `m`，我们得到的结果会系统性地、持续地**低估**真实的总体方差 $\sigma^2$。这种系统性的误差，在统计学上就称为**偏差 (Bias)**。
>
> 3. 解决方案：使用 `m-1` 进行无偏估计
>
> 为了修正这种系统性的低估，统计学家证明，我们应该用 `m-1` 来代替 `m` 作为分母。
>
> **样本方差的无偏估计公式：**
...
>
> 通过将分母减小（从 `m` 变为 `m-1`），我们人为地将计算结果调大了一点。这个调整的幅度恰好可以抵消掉由于使用样本均值 $\bar{x}$ 所带来的低估偏差。
>
> 经过这样校正后，得到的样本方差 $s^2$ 就成了一个**无偏估计量 (Unbiased Estimator)**。这意味着，如果我们从同一个总体中反复抽取大量样本，并为每个样本都用 `m-1` 公式计算方差，那么所有这些样本方差的平均值将会非常接近真实的总体方差 $\sigma^2$
>
> 通过**自由度**直观解释：
>
> 最初，我们有 `m` 个独立的样本观测值，可以说我们有 `m` 个“自由度”，但是，为了计算方差，我们必须**先计算出样本均值 $\bar{x}$**，一旦样本均值 $\bar{x}$ 被确定下来，这 `m` 个样本值就失去了一部分“自由”，它们受到了一个约束：它们的总和必须等于 $m \times \bar{x}$
> 这意味着，如果我们知道了前 `m-1` 个样本值和样本均值，那么最后一个样本值就完全被确定了，它别无选择，不再“自由”
> 因此，在估算样本的内部变异程度（即方差）时，真正能够提供独立信息的数据点只有 `m-1` 个
> 
>所有主流的科学计算库（如 Python 的 `numpy.cov`, `pandas.DataFrame.cov` 等）默认都是使用 m-1 来计算协方差的，正是因为它们的设计初衷是为了满足数据分析和推断性统计的需求
