...
$$
Gini\_Gain = Gini(D_{父}) - \left[ \frac{|D_{左}|}{|D_{父}|} Gini(D_{左}) + \frac{|D_{右}|}{|D_{父}|} Gini(D_{右}) \right]
$$
...
>
> 2.  **计算子节点的Gini**:
>     - $Gini(左) = 1 - (1^2 + 0^2) = 0$  （完全纯净）
>     - $Gini(右) = 1 - ( (1/5)^2 + (4/5)^2 ) = 1 - (0.04 + 0.64) = 0.32$
>
> 3.  **计算加权Gini**:
...
$$
MSE\_Reduction = MSE(D_{父}) - \left[ \frac{N_{左}}{N_{父}} MSE(D_{左}) + \frac{N_{右}}{N_{父}} MSE(D_{右}) \right] 
$$
...
>     - $MSE(根) = \frac{1}{5}[(80-140)^2 + ... + (200-140)^2] = \frac{1}{5}[3600+2500+100+1600+3600] = 2280$
>
> 2.  **尝试以“面积 <= 100”为切分点**:
...
>
> 3.  **计算子节点的MSE**:
>     - 左节点均值 $\bar{y}_{左} = (80+90)/2 = 85$。$MSE(左) = \frac{1}{2}[(80-85)^2 + (90-85)^2] = 25$
>     - 右节点均值 $\bar{y}_{右} = (150+180+200)/3 \approx 176.7$。$MSE(右) = \frac{1}{3}[(150-176.7)^2 + ...] \approx 422.2$
>
> 4.  **计算MSE缩减量**:
...
def run_classification_example():
    ...
    执行并展示决策树分类的示例。
    """
    print("--- 决策树分类示例 (已增加噪声数据) ---")

    # 1. 创建并准备数据
    data = {
        '年龄': [22, 35, 48, 25, 52, 60, 31, 45, 29, 55, 28, 58],
        '薪水(千元)': [50, 80, 120, 60, 150, 200, 75, 130, 70, 160, 110, 95],
        '婚姻状况': ['未婚', '已婚', '已婚', '未婚', '已婚', '已婚', '未婚', '已婚', '未婚', '离异', '未婚', '离异'],
        '是否购买': [0, 1, 1, 0, 1, 1, 0, 0, 0, 1, 0, 1]  # 0代表不购买, 1代表购买
    }
    df = pd.DataFrame(data)

    # 使用独热编码 (One-Hot Encoding)
    # 将'婚姻状况'这样的类别变量转换为模型可以理解的数值变量
    df_processed = pd.get_dummies(df, columns=['婚姻状况'])

    # 定义特征(X)和目标(y)，注意X现在包含了新的独热编码列
    X = df_processed.drop('是否购买', axis=1)
    y = df_processed['是否购买']
    ...
    # 计算并打印模型的准确率
    accuracy = accuracy_score(y_test, y_pred)
    print(f"分类树在测试集上的准确率: {accuracy:.2f}")

    # 4. 对新数据进行预测
    # 假设有一个新客户：年龄38，薪水90k，婚姻状况为'已婚'
    ...
    # 【新增步骤】对新数据应用相同的独热编码
    new_customer_processed = pd.get_dummies(new_customer_raw, columns=['婚姻状况'])
    # 使用reindex确保新数据的列与训练数据完全一致，缺失的列用0填充
    new_customer_aligned = new_customer_processed.reindex(columns=X_train.columns, fill_value=0)

    prediction = clf.predict(new_customer_aligned)
    print(f"预测新客户是否会购买: {'会' if prediction[0] == 1 else '不会'}")

    # 5. 可视化决策树
    plt.figure(figsize=(20, 12))
    plot_tree(clf,
              feature_names=X.columns,
              class_names=['不购买', '购买'],  # 对应0和1
              filled=True,  # 填充颜色
              rounded=True,  # 节点框使用圆角
              fontsize=10)
    plt.title("决策树分类可视化", fontsize=16)
    plt.show()
...
def run_regression_example():
    ...
    # 1. 创建并准备数据
    # 创建虚拟数据：学习时长(X)和考试分数(y)
    rng = np.random.RandomState(42)
    X = np.sort(10 * rng.rand(80, 1), axis=0)
    # 使用指数函数模拟学习饱和曲线，并添加一些随机噪声
    y = 100 * (1 - np.exp(-0.4 * X.ravel())) + rng.normal(0, 5, X.shape[0])

    # 2. 初始化并训练模型
    # 我们同样使用CART回归树，并设置最大深度以防过拟合
    ...
    # 3. 进行预测与评估
    # 对整个数据范围进行预测，以便于可视化
    X_test_range = np.arange(0.0, 10.0, 0.01)[:, np.newaxis]
    y_pred = regr.predict(X_test_range)

    # 计算在原始数据集上的均方误差
    y_original_pred = regr.predict(X)
    mse = mean_squared_error(y, y_original_pred)
    print(f"回归树在训练集上的均方误差(MSE): {mse:.2f}")
    print(f"回归树在训练集上的均方根误差(RMSE): {np.sqrt(mse):.2f}")

    # 4. 对新数据进行预测
    # 假设有一个新学生学习了4.5小时
    new_student_hours = [[4.5]]
    predicted_score = regr.predict(new_student_hours)
    print(f"预测学习4.5小时的学生分数: {predicted_score[0]:.2f}")

    # 5. 可视化
    # 可视化1：绘制数据点和回归树的预测线
    plt.figure(figsize=(10, 6))
    plt.scatter(X, y, s=20, edgecolor="black", c="cornflowerblue", label="原始数据点")
    plt.plot(X_test_range, y_pred, color="orange", label="回归树预测 (max_depth=3)", linewidth=2)
    plt.xlabel("学习时长 (小时)")
    plt.ylabel("考试分数")
    plt.title("决策树回归预测结果", fontsize=16)
    plt.legend()
    plt.show()
    ...
    plot_tree(regr,
              ...
              rounded=True,
              fontsize=10)
    plt.title("决策树回归结构可视化", fontsize=16)
    plt.show()
