---
index_img: 'https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-05-31-1748685017800.webp'
banner_img: 'https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-05-31-1748684560329.webp'
title: 数模笔记模型篇二
categories:
  - 学习笔记
tags:
...
![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-18-1752827020229.webp)

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-18-1752827034845.webp)

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-18-1752827051978.webp)

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-18-1752827074763.webp)
...
![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-22-1753176646597.webp)
...
支持向量机是一种功能强大且用途广泛的监督学习算法，核心为不仅要找到一条能分开两类点的线（或更高维度的**超平面**），还要让这条线到两边数据点的**间隔（Margin）**尽可能宽，使得SVM具有很强的泛化能力，即对新数据的预测能力更强

<img src="https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753330171954.webp" style="zoom: 50%;" />

**超平面（Hyperplane）**: 在二维空间里是**一条直线**，在三维空间里是一个**平面**，在更高维度的空间中，这个分界被称为超平面。它是决策的边界

**间隔（Margin）**：决策边界与离它最近的样本点之间的距离。SVM的目标就是**最大化这个间隔**

**支持向量（Support Vectors）**：那些**离决策边界最近**的样本点（即图中分割线上的点），它们像“支撑”着整个决策边界一样，决定了间隔的宽度和超平面的最终位置。如果移动支持向量，超平面就会改变；如果移动其他非支持向量，超平面则不会受到影响。这是SVM高效的关键之一

**硬间隔与软间隔：**

<img src="https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753330447161.webp" style="zoom: 67%;" />

* 硬间隔完美分类所有点，同时最大化间隔，要求数据必须是**完全线性可分**的
* 软间隔在最大化间隔和最小化分类错误之间找到平衡，可以处理**非线性可分**或**有噪声**的数据，鲁棒性更强

### 核心内容推导

#### 模型定义

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753330709899.webp)

优化目标：最大化间隔

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753330873243.webp)

**硬间隔**的约束

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753330920209.webp)
...
即为**原始可行性条件(Primal Feasibility)**

然后就得到了以下的模型定义，即优化问题，再构建**拉格朗日函数**

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753331128944.webp)

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753331326303.webp)
...
> 含义：定义了 SVM 的“间隔”。要求每个样本点 $i$ 的函数间隔 $y_i(w \cdot x_i + b)$ 必须 $\ge 1$，即所有样本点都被正确分类，且位于间隔边界上或之外
...
$$
w - \sum_{i=1}^{N}\lambda y_i x_i = 0
\quad\Longrightarrow\quad
w = \sum_{i=1}^{N}\lambda_i y_i x_i
$$

> 来源：拉格朗日函数 $L$ 对变量 $w$ 求偏导并令其为零 $\frac{\partial L}{\partial w}=0$
> 含义：决策边界的法向量 $w$ 可表示为所有训练样本向量 $x_i$ 的加权线性组合；权重 $\alpha_i$ 为拉格朗日乘子。只有支持向量的 $\alpha_i>0$，其余为零，因此 $w$ 完全由支持向量决定
...
> 来源：拉格朗日函数 $L$ 对变量 $b$ 求偏导并令其为零 $\frac{\partial L}{\partial b}=0$
> 含义：对偶问题的约束条件，要求所有拉格朗日乘子与对应标签乘积之和为零

3. 互补松弛条件（Complementary Slackness）

$$
\lambda_i\,\bigl[\,y_i(w \cdot x_i + b) - 1\,\bigr] = 0,\quad i = 1,\dots,N
$$

> 来源：KKT 核心条件
> 含义：对任意样本点，必有下列两种情形之一：  
>
> - **情形 A：** $\lambda_i = 0$ ，该点 **不是** 支持向量，对决策边界无贡献；通常远离间隔边界且已正确分类
> - **情形 B：** $y_i(w \cdot x_i + b) - 1 = 0$ ，该点恰位于间隔边界上，此时 $\lambda_i > 0$，即为 **支持向量（Support Vectors）**
...
构造下界函数`q(λ)`，令该函数一定**小于**目标函数的最小值，即 $q(\lambda) \leq f(w^*)$，通过求`q(λ)`最大值得到目标函数$f(w^*)$的最小值，从而将求**最小值**的原问题转换成求**最大值**的对偶问题

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753352988984.webp)

其中所有SVM问题（包括软&硬间隔）都是**强对偶性**，属于凸优化且满足斯莱特条件，因此**max(q) = min(f)**

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753362784594.webp)

将上述KKT条件全部代入后得到最终的对偶问题为：
- **Maximize:**
  $$
  q(\lambda) = \sum_{i=1}^{s} \lambda_i - \frac{1}{2} \sum_{i=1}^{s} \sum_{j=1}^{s} \lambda_i \lambda_j y_i y_j (x_i \cdot x_j)
  $$

- **Subject to:**
  
  1. $\sum_{i=1}^{s} \lambda_i y_i = 0$
  2. $\lambda_i \geq 0, \quad i = 1, 2, \ldots, s$

#### 核方法

当原数据难以被超平面划分时，可以通过**升维**来提高区分度

<img src="https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753368864975.webp" style="zoom:80%;" />

**核技巧：**通过核函数直接计算出高维空间中的点积结果，从而避免对原始数据点进行升维计算

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753369042204.webp)

多项式核

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753369240624.webp)

高斯核
$$
K(\vec{x}_i, \vec{x}_j) = e^{-\gamma \lVert \vec{x}_i - \vec{x}_j \rVert^2}
$$
![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753369352455.webp)

> 其中 $\gamma$ 的值影响着不同距离下数据点的区分度，效果类似于KNN中的k值

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-24-1753369485113.webp)
...
* **目标函数**：
$$
\max_{\boldsymbol{\lambda}} \quad q(\boldsymbol{\lambda}) = \sum_{i=1}^{s} \lambda_i - \frac{1}{2}\sum_{i=1}^{s}\sum_{j=1}^{s} \lambda_i \lambda_j y_i y_j K(\vec{x_i}, \vec{x_j})
$$
...
* **约束条件**：
    ...
    2.  $0 \le \lambda_i \le C \quad (\text{对于软间隔SVM})$

获得最优解 $\lambda_i^*$：

​	将上述二次规划（QP）问题输入优化求解器，得到最优的拉格朗日乘子向量 $\boldsymbol{\lambda}^* = (\lambda_1^*, \lambda_2^*, ..., \lambda_s^*)$

计算偏置项 $b$

1.  识别支持向量 (Support Vectors, SV)
    * 根据求解出的 $\boldsymbol{\lambda}^*$，所有满足 $\lambda_i^* > 0$ 的样本点 $\vec{x_i}$ 都是支持向量

2.  选择一个边界上的支持向量
    * 选择一个满足 $0 < \lambda_s^* < C$ 的支持向量 $(\vec{x_s}, y_s)$。（这保证了该点严格在间隔边界上）

3.  计算 $b$
    * 利用KKT条件 $y_s (\vec{w} \cdot \phi(\vec{x_s}) + b) = 1$ 和关系式 $\vec{w} = \sum_{i=1}^{s} \lambda_i^* y_i \phi(\vec{x_i})$，推导出 $b$ 的计算公式：
        $$
        b = y_s - \sum_{i \in \text{SV}} \lambda_i^* y_i K(\vec{x_i}, \vec{x_s}) 
        $$

> 为提高稳健性，实践中通常会计算所有边界支持向量的 $b$ 值，然后取其平均值。

#### 决策函数

1. 写出决策函数的形式
   
    * 分类新样本 $\vec{x}_{new}$ 的决策函数为：
        $$
        f(\vec{x}_{new}) = \vec{w} \cdot \phi(\vec{x}_{new}) + b
        $$
    
2. 代入并使用核函数替换
    * 将 $\vec{w}$ 的表达式代入，并应用核技巧：
$$
f(\vec{x}_{new}) = \left( \sum_{i \in \text{SV}} \lambda_i^* y_i \phi(\vec{x_i}) \right) \cdot \phi(\vec{x}_{new}) + b 
$$
$$
\Downarrow
$$
$$
f(\vec{x}_{new}) = \sum_{i \in \text{SV}} \lambda_i^* y_i K(\vec{x_i}, \vec{x}_{new}) + b
$$
...
* 最终的分类规则是判断决策函数的符号：
$$
\text{Predicted Class} = \text{sign}(f(\vec{x}_{new}))
$$
* 如果 $f(\vec{x}_{new}) > 0$，预测为正类 (+1)
* 如果 $f(\vec{x}_{new}) < 0$，预测为负类 (-1)

#### 软间隔

* **松弛变量** $\xi_i$：为每个样本点 $\vec{x_i}$ 引入一个非负的松弛变量（$\xi_i \ge 0$），用以衡量该样本点违反间隔约束的程度
  
    * $\xi_i = 0$ 表示样本点遵守规则，位于间隔边界上或之外
    * $0 < \xi_i \le 1$ 表示样本点进入了间隔区域，但仍被正确分类
    * $\xi_i > 1$ 表示样本点被错误分类
    
    > 对于于一个已经训练好的SVM模型（即我们已经找到了最优的 $w^*$ 和 $b^*$），$\xi_i$ 的值为：
    >
    > $$
    > \xi_i = \max(0, 1 - y_i(w^* \cdot x_i + b^*))
    > $$
    
* **惩罚系数** $C$：一个需要手动设置的超参数（$C > 0$），用于控制对松弛量的惩罚力度，它在“最大化间隔”和“最小化分类错误”两个目标之间进行权衡
  
    * 高 $C$ 值意味着对错误的惩罚很重，模型会更倾向于将所有点正确分类，容易过拟合
    * 低 $C$ 值意味着对错误的惩罚较轻，模型会更注重获得一个宽的间隔，容忍更多错误，泛化能力可能更强

**软间隔的优化目标**

* 目标函数：
$$
\min_{\vec{w}, b, \boldsymbol{\xi}} \quad \frac{1}{2}||\vec{w}||^2 + C \sum_{i=1}^{s} \xi_i
$$

* 约束条件：
$$
y_i(\vec{w} \cdot \vec{x_i} + b) \ge 1 - \xi_i
$$
$$
\xi_i \ge 0, \quad i=1, 2, ..., s
$$

4.  对偶问题与支持向量的变化

    对偶问题的主要变化在于拉格朗日乘子 $\lambda_i$ 的约束从 $\lambda_i \ge 0$ 变为了有上界的约束：
$$
0 \le \lambda_i \le C
$$
...
* 这个变化也丰富了支持向量（SV）的类型：
    * $\lambda_i = 0$：非支持向量
    * $0 < \lambda_i < C$：边界上的支持向量，这些点严格位于间隔边界上（$\xi_i = 0$）
    * $\lambda_i = C$：被约束的支持向量，这些点是违反了间隔约束的点（$\xi_i > 0$）
...
#### 核心原理
该方法的理论基础是概率论中的**大数定律**：当样本数量足够大时，一个随机事件出现的频率会无限接近其真实概率。通过构造一个与问题解相关的随机试验，并进行海量重复，最终得到的统计平均值会收敛到问题的真实解

#### 主要特点
* **依赖随机性**：使用伪随机数序列 (Pseudo-random Numbers) 来模拟随机过程
* **概率收敛**：结果是统计意义上的近似解，伴随着一定的统计误差
* **收敛速度**：误差收敛速度通常为 $O(1/\sqrt{N})$，其中 $N$ 是试验次数。这意味着为将误差减半，需要将计算量增加四倍
...
#### 核心原理
该方法用**低差异序列 (Low-discrepancy Sequences)** 取代了伪随机数。所谓“低差异”，指的是点集在空间中分布得非常均匀，系统性地避免了随机抽样中可能出现的点“聚集”和“空白”区域。这种均匀性使得用更少的样本点就能达到对整个样本空间的有效覆盖

#### 主要特点
* **确定性序列**：使用哈尔顿 (Halton)、索博尔 (Sobol') 等确定性的拟随机数序列
* **均匀覆盖**：其设计目标是让样本点尽可能均匀地填充空间，新生成的点总会落在现有最“空旷”的位置
* **收敛速度**：在理想条件下，其误差收敛速度可接近 $O(1/N)$，远快于标准蒙特卡洛方法
...
**定义**: 系统在 $t+1$ 时刻的状态，只取决于它在 $t$ 时刻的状态，而与 $t$ 时刻之前的所有状态都无关
$$
...
- **状态空间 ($S$)**: 系统所有可能状态的集合
- **示例**:
    - **天气模型**: 状态空间 $S = \{\text{晴天, 阴天, 雨天}\}$

#### 转移概率 (Transition Probability) 与转移矩阵 ($P$)
系统从一个状态到另一个状态的可能性。这些概率被组织在一个称为**转移矩阵 $P$** 的方阵中

**矩阵元素**: $p_{ij}$ 表示系统从状态 $i$ **经过一步**转移到状态 $j$ 的概率
...
2.  **行和为1**: $\sum_{j} p_{ij} = 1$。

#### 1. n阶转移矩阵 ($P^{(n)}$)
描述系统**经过 n 步**转移的概率矩阵，即$P^{(n)} = P^n$ (1阶转移矩阵的 n 次幂

- **示例 (续)**: 想知道如果周一晴天，周三（2天后）下雨的概率是多少？我们需要计算 $P^{(2)} = P^2$
  $$
  ...
  \end{pmatrix}
  $$
  矩阵第1行第3列的元素 $p_{13}^{(2)} = 0.04$，意味着周一晴天，周三下雨的概率是4%

#### 2. 切普曼-柯尔莫哥洛夫方程 (Chapman-Kolmogorov Equation)
- **公式**: $P^{(m+n)} = P^{(m)} P^{(n)}$
- **解释**: 从状态 $i$ 走 $m+n$ 步到状态 $j$ 的过程，可以分解为：先用 $m$ 步从 $i$ 走到某个中间状态 $k$，再用 $n$ 步从 $k$ 走到 $j$。这个方程将所有可能的中间路径 $k$ 的概率加和起来，其结果恰好是矩阵乘法


### 链的分类与状态的性质

#### 1. 可约性 (Reducibility)
- **不可约 (Irreducible)**: 链中任意两个状态都是**互通**的（可以相互到达）。系统像一个设计良好的城市，所有地点之间都有路径
...
- **可约 (Reducible)**: 链中存在不互通的状态。系统内部存在“孤岛”或“陷阱”
    - **示例：带吸收态的游戏**
      ...
      \end{pmatrix}
      $$
      一旦进入状态2“胜利”或状态3“失败”，就再也出不来 ($p_{22}=1, p_{33}=1$)。状态1可以到2和3，但反过来不行。因此这个链是**可约**的

#### 2. 周期性 (Periodicity)
- **非周期 (Aperiodic)**: 状态的返回没有固定的节奏
  
    - **示例**: 在天气模型中，从“晴天”出发，可能1步后（持续晴天）、2步后、3步后...都能返回“晴天”，这些返回步数(1,2,3,...)的最大公约数是1，所以是**非周期**的
...
- 在上面可约的游戏示例中，状态2和3是**常返态**（甚至是吸收态）。状态1是**瞬时态**，因为只要离开状态1，就再也回不来了。

#### 4. 平稳分布 (Stationary Distribution)

当一个马尔科夫链运行足够长时间后，系统处于各个状态的概率将不再随时间变化，达到一种统计上的平衡。这个平衡时的概率分布，就是**平稳分布 $\pi$**
...
> 平稳分布向量$\pi$即特征值为1的特征向量

当前的概率分布 $\pi$ 经过一次转移（乘以P）后，得到的下一时刻的分布依然是 $\pi$

对于一个**有限状态、不可约、非周期**的马尔科夫链，**存在唯一**的平稳分布 $\pi$

**与极限矩阵的关系：**
- 对于满足上述黄金定理的链，当 $n \to \infty$ 时， $P^{(n)}$ 会收敛到一个所有行都完全相同的矩阵 $W$
- 这个矩阵的每一行，就是那个**唯一的平稳分布向量 $\pi$**
- **深刻含义**: 经过足够长的时间，系统**遗忘了它的初始状态**。无论一开始是晴天、阴天还是雨天，在遥远的未来，是晴天的概率都会趋近于同一个值 $\pi_1$
- **示例(续)**：对于我们的天气模型，可以解出其唯一平稳分布约为 $\pi \approx (0.655, 0.259, 0.086)$。这意味着，长期来看，这个地区大约65.5%的日子是晴天，25.9%是阴天，8.6%是雨天

## HMM 隐马尔可夫模型

隐马尔可夫模型 (Hidden Markov Model, HMM) 是标准马尔科夫链的扩展，用于解决**状态不可观测**的问题。它描述了这样一个过程：系统内部存在一个不可见的、隐藏的马尔科夫状态链，而我们只能观测到由这些隐藏状态以一定概率生成的输出序列。HMM的目标就是通过可观测的序列，来推断隐藏状态的性质和序列
...
![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-27-1753548024553.webp)

![](https://mirrors.sustech.edu.cn/git/giraffish/image-hosting/-/raw/main/blog/25-07-27-1753548074139.webp)

###  HMM 的五大组成要素

一个完整的HMM模型由一个五元组 $\lambda = (S, O, A, B, \pi)$ 定义：

1.  **隐藏状态集合 $S$**:
    -   所有可能的隐藏状态的集合
    -   $S = \{s_1, s_2, ..., s_N\}$，其中 $N$ 是隐藏状态的数量

2.  **观测集合 $O$**:
    -   所有可能的观测值的集合。
    -   $O = \{o_1, o_2, ..., o_M\}$，其中 $M$ 是观测值的数量

3.  **状态转移概率矩阵 $A$**:
    -   描述**隐藏状态之间**的转移概率，与标准马尔科夫链的转移矩阵相同
    -   $A = [a_{ij}]$，其中 $a_{ij} = P(\text{下一时刻为状态 } s_j | \text{当前为状态 } s_i)$

4.  **发射概率矩阵 $B$ (观测概率矩阵)**:
    -   描述在某个**隐藏状态**下，生成某个**观测值**的概率。这是HMM特有的核心
    -   $B = [b_j(k)]$，其中 $b_j(k) = P(\text{观测到 } o_k | \text{当前为状态 } s_j)$

5.  **初始状态概率分布 $\pi$**:
    -   描述系统在初始时刻处于各个**隐藏状态**的概率
    -   $\pi = [\pi_i]$，其中 $\pi_i = P(\text{初始时刻为状态 } s_i)$
...
| 问题类型                  | 问题描述                                                     | 解决方法                                   |
| :------------------------ | :----------------------------------------------------------- | :----------------------------------------- |
| **评估问题 (Evaluation)** | 给定模型 $\lambda$ 和观测序列 $O$，计算该序列出现的总概率 $P(O|\lambda)$ | **前向算法 (Forward Algorithm)**           |
| **解码问题 (Decoding)**   | 给定模型 $\lambda$ 和观测序列 $O$，找到**最有可能**产生此观测的隐藏状态序列 $Q$ | **维特比算法 (Viterbi Algorithm)**         |
| **学习问题 (Learning)**   | 仅给定观测序列 $O$，如何调整模型参数 $\lambda=(A, B, \pi)$，使其最能解释观测数据 | **鲍姆-韦尔奇算法 (Baum-Welch Algorithm)** |