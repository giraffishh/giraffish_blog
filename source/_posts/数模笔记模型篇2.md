...
def check_cuda_support():
    """检查系统的CUDA支持情况"""
    try:
        # 尝试创建一个简单的GPU训练来测试CUDA支持
        import xgboost as xgb
        dtrain = xgb.DMatrix([[1, 2], [3, 4]], label=[0, 1])
        model = xgb.train({'tree_method': 'hist', 'device': 'cuda'}, dtrain, num_boost_round=1)
        print("✅ CUDA 加速可用，XGBoost 将使用 GPU 进行训练\n")
        return True
    except Exception as e:
        print("❌ CUDA 加速不可用，将使用 CPU 训练")
        print(f"❌ 错误信息: {e}\n")
        return False
...
# ==============================================================================
def run_iris_classification(cuda_available=False):
    """
    ...
    shap_model_iris = xgb.XGBClassifier(
        ...
        device='cpu', tree_method='hist', **best_params_iris
    )
...
# ==============================================================================
def run_california_housing_regression(cuda_available=False):
    """
    ...
    shap_model_housing = xgb.XGBRegressor(
        ...
        device='cpu', tree_method='hist', **best_params_housing
    )
...
if __name__ == '__main__':
    ...
    run_iris_classification(cuda_available)
    run_california_housing_regression(cuda_available)
```