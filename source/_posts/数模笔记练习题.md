...
```python
from unified_optimization import OptimizationSolver
...
2.3 某公司新购置了某种设备6台，欲分配给下属的4个企业，已知各企业获得这种设备后年创利润如表2.2所列（单位：千万元）。每个企业至少分配1台设备，问应如何分配这些设备能使年创总利润最大，最大利润是多少？
...
x_{ij} =
\begin{cases}
1, & \text{如果将第 }i\text{ 台设备分配给第 }j\text{ 个企业} \\
0, & \text{否则}
\end{cases}
$$

其中 $i = 1, 2, \dots, 6$，$j = 1, 2, 3, 4$ 分别代表设备编号和企业（甲、乙、丙、丁）。

**目标函数：**

最大化利润总和：
$$
\max \sum_{i=1}^{6} \sum_{j=1}^{4} p_{ij} \cdot x_{ij}
$$

其中 $p_{ij}$ 为第 $i$ 台设备分配给第 $j$ 个企业所产生的利润（单位：千万元），给定如下：
...
1. 每台设备只能分配给一个企业
$$
\sum_{j=1}^4 x_{ij} = 1,\quad \forall i = 1, 2, \dots, 6
$$

2. 每个企业至少获得一台设备
$$
\sum_{i=1}^6 x_{ij} \geq 1,\quad \forall j = 1, 2, 3, 4
$$

3. 变量为二进制
$$
x_{ij} \in \{0, 1\},\quad \forall i = 1, 2, \dots, 6;\; j = 1, 2, 3, 4
$$
...
```python
from unified_optimization import OptimizationSolver  # Assuming your class is saved here
...
# 创建整数变量：x[i][j] 表示设备 i 是否分配给企业 j
solver = OptimizationSolver("Equipment Allocation")
solver.create_model(n_vars=num_equipments * num_enterprises,
                    var_names=[f"x_{i+1}_{j+1}" for i in range(num_equipments) for j in range(num_enterprises)],
                    var_types='binary')

# 定义目标函数：最大化总利润
def objective(model):
    return sum(profits[i][j] * model.x[i * num_enterprises + j + 1] for i in range(num_equipments) for j in range(num_enterprises))

solver.set_objective(objective, sense='maximize')

# 每个设备只能分配给一个企业
for i in range(num_equipments):
    solver.add_constraint(lambda model, i=i: sum(model.x[i * num_enterprises + j + 1] for j in range(num_enterprises)) - 1, sense='=')

# 每个企业至少分配1台设备
for j in range(num_enterprises):
    solver.add_constraint(lambda model, j=j: sum(model.x[i * num_enterprises + j + 1] for i in range(num_equipments)) - 1, sense='>=')

# 求解问题
solver.solve()
solver.print_solution()

total_profit = solver.get_solution()['objective_value']
print(f"\n💰 最大利润为: {total_profit:.2f} 千万元")
```
...
3.1 某工厂向用户提供发动机，按合同规定，其交货数量和日期是：第一季度末交40台，第二季末交60台，第三季末交80台。工厂的最大生产能力为每季100台，每季的生产费用是 $f(x) = 50x + 0.2x^2$（元），此处 $x$ 为该季生产发动机的台数。若工厂生产得多，多余的发动机可移到下季向用户交货，这样，工厂就需支付存储费，每台发动机每季的存储费为4元。问该厂每季应生产多少台发动机，才能既满足交货合同，又使工厂所花费的费用最少（假定第一季度开始时发动机无存货）？
...
- $x_1, x_2, x_3$：第1、2、3季度的生产数量
- $s_1, s_2$：第1、2季度末的库存量

**目标函数：**
最小化总费用
$$
W = 50x_1 + 0.2x_1^2 + 50x_2 + 0.2x_2^2 + 50x_3 + 0.2x_3^2 + 4s_1 + 4s_2
$$
...
```python
from unified_optimization import OptimizationSolver  # Assume your class is saved and imported

# Initialize solver
solver = OptimizationSolver("Engine_Production")
solver.create_model(n_vars=5,  # x1, x2, x3, s1, s2
                    var_names=['x1', 'x2', 'x3', 's1', 's2'],
                    var_types='continuous',
                    lb=[0, 0, 0, 0, 0],
                    ub=[100, 100, 100, None, None])

# Objective: total production + inventory cost
def objective(model):
    return (50 * model.x[1] + 0.2 * model.x[1]**2 +
            50 * model.x[2] + 0.2 * model.x[2]**2 +
            50 * model.x[3] + 0.2 * model.x[3]**2 +
            4 * model.x[4] + 4 * model.x[5])

solver.set_objective(objective, 'minimize')

# x1 = 40 + s1
solver.add_constraint(lambda m: m.x[1] - 40 - m.x[4], sense='=')
# x2 + s1 = 60 + s2
solver.add_constraint(lambda m: m.x[2] + m.x[4] - 60 - m.x[5], sense='=')
# x3 + s2 = 80
solver.add_constraint(lambda m: m.x[3] + m.x[5] - 80, sense='=')

# Solve
if solver.solve():
    solver.print_solution()
else:
    print("❌ Failed to solve the engine production problem.")
